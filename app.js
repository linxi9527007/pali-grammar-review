...
async function init(){
  GRAMMAR=await (await fetch('grammar.json?v=17.6',{cache:'no-store'})).json();
...
async function forceClearAllCaches(){
  try{
    ...
    location.href='./index.html?v=17.6&cache=cleared&ts='+Date.now();
  }catch(e){
...
(function(){
  ...
  window.addEventListener('DOMContentLoaded',function(){document.querySelectorAll('button').forEach(b=>{if(!b.type)b.type='button';});const badge=document.querySelector('.visual-version-badge');if(badge)badge.textContent='Pāli Learning Lab · 17.6 巴利词点击查词版';updateLessonNav();renderGlobalSearchFull();});
...
(function(){
  ...
  window.addEventListener('DOMContentLoaded',function(){
    ...
    if(badge)badge.textContent='Pāli Learning Lab · 17.6 巴利词点击查词版';
...
(function(){
  ...
  function isPaliWord176(w){
    ...
    if(PALI_DIACRITIC_RE.test(x))return true;
    return PALI_COMMON.has(x.toLowerCase());
  }
